...
import { GenerationDialogRestore } from '~/components/common/dialog/generation'
...
const valueStartDate = extractValue($generationStore.trashGenerations, 'startDate')
const valueEndDate = extractValue($generationStore.trashGenerations, 'endDate')
...
async function fetchData() {
  await Promise.all([
    $generationStore.trashGeneration()
  ])
}
</script>
...
<template>
  <div class="w-full flex flex-col gap-4">
    <div class="flex justify-between items-center">
      <h2 class="text-4xl font-bold tracking-tight">Thùng rác khóa học</h2>

      <div class="flex justify-between items-center gap-4">
        <NuxtLink to="/admin/generations">
    ...
    <LayoutTable
      :data="$generationStore.trashGenerations"
      :columns="columns"
      :filters="filters"
    />
  </div>
  ...
  <Dialog :open="isRestoring" @update:open="handleCloseDialog">
    <DialogContent class="sm:max-w-[425px]" @interact-outside="handleInteractOutside">
      <GenerationDialogRestore :data="selectedValue" />
    </DialogContent>
  </Dialog>
</template>
