...
const deleteMaterial = (item: any) => {
  $bus.emit('open-dialog-delete-material-block', { isDeleting: true, item })
}
</script>
...
<template>
  ...
  >
    <Card v-for="(item, index) in props.data" :key="index" class="transition-all duration-300 ease-in-out shadow-md hover:shadow-xl hover:bg-muted/50 cursor-pointer">
      <CardHeader class="flex flex-row justify-between items-start gap-4">
        <div class="flex flex-col gap-1">
          ...
          <CardDescription>
            {{ item.subjectName }} - {{ item.blockName }}
          </CardDescription>
        </div>
